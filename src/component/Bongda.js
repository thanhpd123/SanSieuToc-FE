...
const Bongda = () => {
  ...
  // Lọc sân bóng đá theo từ khóa truyền từ Home (nếu có)
  useEffect(() => {
    let result = fields.filter(
      (field) => field.type?.name?.toLowerCase() === "bóng đá"
    );
    ...
    if (searchPrice) {
      // Nếu muốn lọc chính xác giá:
      // const priceNum = Number(searchPrice);
      // result = result.filter(field => field.pricePerHour === priceNum);

      // Nếu muốn tìm các sân có giá chứa số nhập vào (dạng text):
    ...
    <div className="font-sans">
      ...
      <main className="content-wrapper">
        ...
        <section className="fields-list-container">
          <h1 className="t1">Sân Bóng Đá</h1>
          {loadingFields ? (
