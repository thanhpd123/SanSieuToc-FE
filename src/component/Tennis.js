...
const Tennis = () => {
  ...
  // Lọc sân tennis theo từ khóa truyền từ Home (nếu có)
  useEffect(() => {
    let result = fields.filter(
      (field) => field.type?.name?.toLowerCase() === "tennis"
    );
